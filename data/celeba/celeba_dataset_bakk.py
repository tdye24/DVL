...
def prepare_dataloaders(main_PID, target_PID, num_auxiliary_train_samples=200, num_users=4, batch_size=16):
    ...
    auxiliary_train_img_names = np.hstack((
        np.random.choice(PP, num_auxiliary_train_samples // 4, replace=False),
        np.random.choice(PN, num_auxiliary_train_samples // 4, replace=False),
        np.random.choice(NP, num_auxiliary_train_samples // 4, replace=False),
        np.random.choice(NN, num_auxiliary_train_samples // 4, replace=False),
    ))
    ...
    auxiliary_train_loader = DataLoader(dataset=auxiliary_train_d, batch_size=batch_size, shuffle=True, num_workers=0,
                                        pin_memory=True)

    combined_img_names = np.hstack((PP, PN, NP, NN))
    train_test_candidate_img_names = list(set(combined_img_names) - set(auxiliary_train_img_names))
    assert len(train_test_candidate_img_names) >= 5000 # 4000 train + 1000 test, 4 clients and 1000 per client
    train_test_img_names = np.random.choice(train_test_candidate_img_names, 5000)
